...
def combine_features(tuple_data, params, is_train=False):
    ...
    input_ids = pad_sequences(
        input_ids, maxlen=int(params["max_length"]), dtype="long", value=0, truncating="post", padding="post"
    )
    att_vals = pad_sequences(
        att_vals, maxlen=int(params["max_length"]), dtype="float", value=0.0, truncating="post", padding="post"
    )
